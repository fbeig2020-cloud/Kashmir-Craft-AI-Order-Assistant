...
**AI-Powered E-Commerce Order Operations Assistant** — An AI assistant for managing order issues on the Kashmir Craft website, focusing on safety, reliability, and ease of management.
...
- **REQ-014** — The system must resolve order issues within 2 hours.
...
Who this is for and what they are trying to get done. Take the roles from your own stories — they are written "As a <role>, I want …". Roles in your plan: customer support agent, system auditor.
...
- **REQ-015** — The system must log all actions and decisions for audit purposes.
...
- **r0** Initial Skeleton — 3 stories
- **r1** Evidence Gathering and Recommendation — 4 stories
- **r2** Risk Assessment and Approval — 2 stories
- **r3** Outcome Verification and Processing Control — 2 stories
- **r4** User Experience and Performance — 3 stories
...
- **Customer Support Team** — owns STORY-001, STORY-003, STORY-004, STORY-005, STORY-006, STORY-007, STORY-008, STORY-009, STORY-011, STORY-015
- **System Auditor** — owns STORY-002
- **Order Processing System** — owns STORY-012
- **Inventory Management System** — owns STORY-013
- **Logistics Management System** — owns STORY-014
...
- **REQ-001** (FUNC, must) — The system must identify duplicate orders from the online data source.
- **REQ-002** (FUNC, must) — The system must detect payment failures from the online data source.
- **REQ-003** (FUNC, must) — The system must identify inventory issues from the online data source.
- **REQ-004** (FUNC, must) — The system must detect shipping delays from the online data source.
- **REQ-005** (FUNC, must) — The system must gather evidence for each identified order issue.
- **REQ-006** (FUNC, must) — The system must recommend an action for each order issue.
- **REQ-007** (FUNC, must) — The system must check the risk level of each recommended action.
- **REQ-008** (FUNC, must) — The system must require human approval for high-risk actions.
- **REQ-009** (FUNC, must) — The system must verify the outcome of each action taken.
- **REQ-010** (FUNC, must) — The system must pause processing when unsure about an order issue.
- **REQ-011** (FUNC, must) — The system must provide a summary of actions prepared before execution.
- **REQ-012** (FUNC, must) — The system must be triggered by an order submission event.
- **REQ-013** (NFR, must) — Every screen the customer support team uses must complete its primary action in three clicks or fewer.
- **REQ-014** (NFR, must) — The system must resolve order issues within 2 hours.
- **REQ-015** (SAFE, must) — The system must log all actions and decisions for audit purposes.
- **REQ-016** (CONSTRAINT, must) — The system must connect to the online data source for order information.
...
**r1 · Evidence Gathering and Recommendation**
